...
namespace Mangaya1.Controllers
{
    public class SupportController : Controller
    {
        ...
        public ActionResult profiledata(int supportid)
        {
            ...
            int totalviews = db.translations.Where(x => x.tanslator_id.Equals(supportid)).Select(x => (int?)x.chapter.no_of_views).Sum()??0;
            int totaldownloads= db.translations.Where(x => x.tanslator_id.Equals(supportid)).Select(x =>(int?) x.chapter.no_of_downloads).Sum()??0;
            int totalviewsthismonth = db.translations.Where(x => x.tanslator_id.Equals(supportid) && x.chapter.date.Month == DateTime.Now.Month && x.chapter.date.Year == DateTime.Now.Year).Select(x => (int?)x.chapter.no_of_views).Sum()??0;
            int totaldownloadsthismonth = db.translations.Where(x => x.tanslator_id.Equals(supportid) && x.chapter.date.Month == DateTime.Now.Month && x.chapter.date.Year == DateTime.Now.Year).Select(x => (int?)x.chapter.no_of_downloads).Sum()??0;
            return Json(new {
                supportName = s.name,
